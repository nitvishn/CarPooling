...
def locationFromString(string):
	...
	response = requests.get('https://maps.googleapis.com/maps/api/geocode/json?address=' + string)
...
def sortViables(host, viables):
	...
	for rider in viables:
		hostDestination=Location(host['destinationLat'], host['destinationLong'])
		riderDestination=Location(rider['destinationLat'], rider['destinationLong'])
		dist=computeTotalDistance([origin, riderDestination, hostDestination])
